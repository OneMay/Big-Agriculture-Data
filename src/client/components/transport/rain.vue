    ...
    export default {
        ...
        methods: {
            setData(year, month, day, num) {
                ...
                if (n == 0) {
                    this.option.title.text = (year + '-' + month + '-' + day + 'CO2浓度变化');
                }
                    ...
                    Axios.post(params)
                        .then(res => {
                            ...
                            if (data.rainfallInfo) {
                                this.myecharts.showLoading();
                                //数组去重
                                var newData = [data.rainfallInfo[0].staticRainfallInfo.datas[0]];
                                for (var i = 1; i < data.rainfallInfo[0].staticRainfallInfo.datas.length; i++) {
                                    var repeat = false;
                                    for (var j = 0; j < newData.length; j++) {
                                        if (data.rainfallInfo[0].staticRainfallInfo.datas[i].hour === newData[j].hour) {
                                            repeat = true;
                                            break;
                                        }
                                    }
                                    if (!repeat) {
                                        newData.push(data.rainfallInfo[0].staticRainfallInfo.datas[i])
                                    }
                                }
