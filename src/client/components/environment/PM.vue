<template>
    <div id="PM" :style="{width:'1200px',height:'400px'}"></div>
</template>
    ...
    export default {
        ...
        methods: {
            setData(year, month, day, num) {
                ...
                if (n == 0) {
                    this.option.title.text = (year + '-' + month + '-' + day + 'PM2.5变化');
                } else {
                    this.option.title.text = '最近' + n + '天PM2.5变化';
                }
                    ...
                    Axios.post(params)
                        .then(res => {
                            ...
                            if (data.pmInfo) {
                                this.myecharts.showLoading();
                                //数组去重
                                var newData = [data.pmInfo[0].staticPmInfo.datas[0]];
                                for (var i = 1; i < data.pmInfo[0].staticPmInfo.datas.length; i++) {
                                    var repeat = false;
                                    for (var j = 0; j < newData.length; j++) {
                                        if (data.pmInfo[0].staticPmInfo.datas[i].hour === newData[j].hour) {
                                            repeat = true;
                                            break;
                                        }
                                    }
                                    if (!repeat) {
                                        newData.push(data.pmInfo[0].staticPmInfo.datas[i])
                                    }
                                }
                    ...
                    Axios.post(params)
                        .then(res => {
                            ...
                            for (var i = 0; i < data.pmInfo.length; i++) {
                                for (var j = i + 1; j < data.pmInfo.length; j++) {
                                    if (data.pmInfo[i].date > data.pmInfo[j].date) {
                                        var tmp = data.pmInfo[i];
                                        data.pmInfo[i] = data.pmInfo[j];
                                        data.pmInfo[j] = tmp;
                                    }
                                }
                            }
                            var MaxDatas = [],
                                MinDatas = [],
                                averageDatas = [];
                            data.pmInfo.forEach(function(val, index) {
                                MaxDatas.push(val.MaxData);
                                MinDatas.push(val.MinData);
                                averageDatas.push(val.average);
                            });

                            this.option.legend.data = ['最大PM2.5', '平均PM2.5', '最小PM2.5']
                            this.option.series = [{
                                name: '最大PM2.5',
                                type: 'line',
                                data: MaxDatas,
