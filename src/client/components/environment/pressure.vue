<template>
    <div id="pressure" :style="{width:'1200px',height:'400px'}"></div>
</template>
    ...
    export default {
        ...
        methods: {
            setData(year, month, day, num) {
                ...
                if (n == 0) {
                    this.option.title.text = (year + '-' + month + '-' + day + '气压变化');
                } else {
                    this.option.title.text = '最近' + n + '天气压变化';
                }
            ...
            getData(date, n) {
                if (n == 1) {
                    var time = [date]
                    let params = {
                        api: url + '/api/find/staticPressure',
                        param: {
                            date: time
                        }
                    }
                    Axios.post(params)
                        .then(res => {
                            ...
                            if (data.pressureInfo) {
                                this.myecharts.showLoading();
                                //数组去重
                                var newData = [data.pressureInfo[0].staticPressureInfo.datas[0]];
                                for (var i = 1; i < data.pressureInfo[0].staticPressureInfo.datas.length; i++) {
                                    var repeat = false;
                                    for (var j = 0; j < newData.length; j++) {
                                        if (data.pressureInfo[0].staticPressureInfo.datas[i].hour === newData[j].hour) {
                                            repeat = true;
                                            break;
                                        }
                                    }
                                    if (!repeat) {
                                        newData.push(data.pressureInfo[0].staticPressureInfo.datas[i])
                                    }
                                }
                            ...
                            for (var i = 0; i < data.pressureInfo.length; i++) {
                                for (var j = i + 1; j < data.pressureInfo.length; j++) {
                                    if (data.pressureInfo[i].date > data.pressureInfo[j].date) {
                                        var tmp = data.pressureInfo[i];
                                        data.pressureInfo[i] = data.pressureInfo[j];
                                        data.pressureInfo[j] = tmp;
                                    }
                                }
                            }
                            var MaxDatas = [],
                                MinDatas = [],
                                averageDatas = [];
                            data.pressureInfo.forEach(function(val, index) {
                                MaxDatas.push(val.MaxData);
                                MinDatas.push(val.MinData);
                                averageDatas.push(val.average);
                            });

                            this.option.legend.data = ['最大气压', '平均气压', '最小气压']
                            this.option.series = [{
                                name: '最大气压',
                                type: 'line',
                                data: MaxDatas,
