<template>
    <div id="windspeed" :style="{width:'1200px',height:'400px'}"></div>
</template>
                    ...
                    Axios.post(params)
                        .then(res => {
                            ...
                            if (data.WindspeedInfo) {
                                this.myecharts.showLoading();
                                //数组去重
                                var newData = [data.WindspeedInfo[0].staticWindspeedInfo.datas[0]];
                                for (var i = 1; i < data.WindspeedInfo[0].staticWindspeedInfo.datas.length; i++) {
                                    var repeat = false;
                                    for (var j = 0; j < newData.length; j++) {
                                        if (data.WindspeedInfo[0].staticWindspeedInfo.datas[i].hour === newData[j].hour) {
                                            repeat = true;
                                            break;
                                        }
                                    }
                                    if (!repeat) {
                                        newData.push(data.WindspeedInfo[0].staticWindspeedInfo.datas[i])
                                    }
                                }
