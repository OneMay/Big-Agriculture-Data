    ...
    export default {
        ...
        methods: {
            setData(year, month, day, num) {
                ...
                if (n == 0) {
                    this.option.title.text = (year + '-' + month + '-' + day + '土壤湿度变化');
                } else {
                    ...
                    Axios.post(params)
                        .then(res => {
                            ...
                            if (data.soilhumidityInfo) {
                                this.myecharts.showLoading();
                                //数组去重
                                var newData = [data.soilhumidityInfo[0].staticSoilhumidityInfo.datas[0]];
                                for (var i = 1; i < data.soilhumidityInfo[0].staticSoilhumidityInfo.datas.length; i++) {
                                    var repeat = false;
                                    for (var j = 0; j < newData.length; j++) {
                                        if (data.soilhumidityInfo[0].staticSoilhumidityInfo.datas[i].hour === newData[j].hour) {
                                            repeat = true;
                                            break;
                                        }
                                    }
                                    if (!repeat) {
                                        newData.push(data.soilhumidityInfo[0].staticSoilhumidityInfo.datas[i])
                                    }
                                }
